...
| **Area Type** | Select the [area type](AreasAndCosts.html#area-types) that the NavMesh Modifier Volume applies to NavMeshes within the defined region. The options include all of the area types that have a cost defined in the [Areas tab](NavigationWindow.html#areas-tab) of the Navigation window. Built-in types include the following:<ul><li>**Walkable** is a generic area type which specifies that the area can be walked on.</li><li>**Not Walkable** is a generic area type which prevents navigation. It is useful for cases where you want to mark a certain object as an obstacle, without generating a NavMesh on top of it.</li><li>**Jump** is an area type that is assigned to all auto-generated **NavMesh Links**.</li></ul><br/>Select **Open Area Settings** to open the Areas tab of the Navigation window to define new area types or modify existing ones. |
